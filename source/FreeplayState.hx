...
class FreeplayState extends MusicBeatState
{
	...
	private var AllPossibleSongs:Array<String> = ["Peashooter","Base","Joke","Extra"];
	...
	var songColors:Array<FlxColor> = [
        ...
        0xFFffaa6f, // SENPAI
		0xFF00B515, // DAVE
	    0xFF00FFFF, // MISTER BAMBI RETARD
	    0xFF4965FF  // SPLIT THE THONNNNN
	...
	public function LoadProperPack()
	{
		...
		else if (AllPossibleSongs[CurrentPack].toLowerCase() == 'peashooter')
		{
			addWeek(['House', 'Insanity', 'Furiosity'], 7, ['peashooter', 'peashooter-annoyed', 'peashooter-angey']);
			addWeek(['Bonus-Song'],7,['peashooter']);
			addWeek(['Blocked','Corn-Theft','Maze',], 8, ['snow-pea']);
			addWeek(['Splitathon'],9,['the-duo']);
		}
		else if (AllPossibleSongs[CurrentPack].toLowerCase() == 'joke')
		{
			addWeek(['Supernovae', 'Glitch'], 8, ['snowpea-old']);
			if (FlxG.save.data.cheatingFound)
			{
				addWeek(['Cheating'], 8, ['snowpea-3d']);
			}
		}
		else if (AllPossibleSongs[CurrentPack].toLowerCase() == 'extra')
		{
			addWeek(['Freaky-Menu'], 8, ['snowpea-old']);
			addWeek(['Screwed'], 8, ['snowpea-angey']);
		}
	}
