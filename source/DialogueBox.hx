...
class DialogueBox extends FlxSpriteGroup
{
	...
	public function new(talkingRight:Bool = true, ?dialogueList:Array<String>)
	{
		...
		switch (PlayState.SONG.song.toLowerCase())
		{
			...
			case 'house' | 'insanity' | 'splitathon':
				FlxG.sound.playMusic(Paths.music('PeashooterDialogue'), 0);
				FlxG.sound.music.fadeIn(1, 0, 0.8);
			case 'furiosity':
		...
		switch (PlayState.SONG.song.toLowerCase())
		{
			...
			case 'house' | 'insanity' | 'furiosity':
				portraitLeftCharacter = 'peashooter';
				
			case 'blocked' | 'corn-theft' | 'maze' | 'supernovae' | 'glitch' | 'splitathon':
				portraitLeftCharacter = 'snow-pea';
		}
		...
		switch (PlayState.SONG.song.toLowerCase())
		{
			...
			case 'house' | 'insanity' | 'supernovae' | 'glitch' |  'blocked' | 'corn-theft' | 'maze' | 'splitathon':
				dropText = new FlxText(242, 485, Std.int(FlxG.width * 0.6), "", 32);
				dropText.font = 'Comic Sans MS Bold';
				dropText.color = 0x00FF00;
				add(dropText);
		
				swagDialogue = new FlxTypeText(240, 483, Std.int(FlxG.width * 0.6), "", 32);
				swagDialogue.font = 'Comic Sans MS Bold';
				swagDialogue.color = 0x00CC00; 
	...
	function startDialogue():Void
	{
		...
		if (curCharacter != 'generic')
		{
			...
			switch (curCharacter)
			{
				case 'peashooter' | 'snow-pea': //guys its the funny snow pea character
						portraitLeft.setPosition(220, 210);
			...
			portraitLeft.x -= 150;
			//portraitRight.x += 100;
			portraitLeft.antialiasing = portrait.portraitPath != "dialogue/peashooter_furiosity";
	...
	function getPortrait(character:String):Portrait
	{
		...
		switch (character)
		{
			case 'peashooter':
				...
				{
					case 'house':
						portrait.portraitPath = 'dialogue/peashooter_house';
						portrait.portraitPrefix = 'peashooter house portrait';

					case 'insanity':
						portrait.portraitPath = 'dialogue/peashooter_insanity';
						portrait.portraitPrefix = 'peashooter insanity portrait';

					case 'pre-furiosity LUL':
						portrait.portraitPath = 'dialouge/peashooter_pre-furiosity';
						portrait.portraitPrefix = 'peashooter pre-furiosity portrait';

					case 'furiosity':
						portrait.portraitPath = 'dialogue/peashooter_furiosity';
						portrait.portraitPrefix = 'peashooter furiosity portrait';

					case 'blocked' | 'corn-theft' | 'maze':
						portrait.portraitPath = 'dialogue/peashooter_snowpea_week';
						portrait.portraitPrefix = 'peashooter snow pea week portrait';
					case 'splitathon':
						portrait.portraitPath = 'dialogue/peashooter_splitathon';
						portrait.portraitPrefix = 'peashooter splitathon portrait';
				}
			case 'snow-pea':
				switch (PlayState.SONG.song.toLowerCase())
				{
					case 'blocked':
						portrait.portraitPath = 'dialogue/snow_pea_blocked';
						portrait.portraitPrefix = 'snow pea blocked portrait';
					case 'corn-theft':
						portrait.portraitPath = 'dialogue/snow_pea_corntheft';
						portrait.portraitPrefix = 'snow pea corntheft portrait';
					case 'maze':
						portrait.portraitPath = 'dialogue/snow_pea_maze';
						portrait.portraitPrefix = 'snow pea maze portrait';
					case 'splitathon':
						portrait.portraitPath = 'dialogue/snow_pea_splitathon';
						portrait.portraitPrefix = 'snow pea splitathon portrait';
				}
			case 'senpai':
