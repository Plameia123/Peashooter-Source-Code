...
class PlayState extends MusicBeatState
{
	...
	var funnyFloatyBoys:Array<String> = ['peashooter-angey', 'snowpea-3d'];
	...
	private var SNOWPEACUTSCENEICONHURHURHUR:HealthIcon;
	...
	override public function create()
	{
		...
		{
			case 'peashooter':
				iconRPC = 'icon_peashooter';
			case 'snow-pea' | 'snowpea-old':
				iconRPC = 'icon_snow-pea';
			default:
				iconRPC = 'icon_none';
		}
		...
		else if (SONG.song.toLowerCase() == 'house' || SONG.song.toLowerCase() == 'insanity' || SONG.song.toLowerCase() == 'supernovae' || SONG.song.toLowerCase() == 'freaky-menu')
		{
		    defaultCamZoom = 0.9;
			curStage = 'daveHouse';
			var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('peashooter/sky'));
			bg.antialiasing = true;
			bg.scrollFactor.set(0.9, 0.9);
			bg.active = false;
			add(bg);

			var stageHills:FlxSprite = new FlxSprite(-225, -130).loadGraphic(Paths.image('peashooter/hills'));
			...
			var gate:FlxSprite = new FlxSprite(-225, 23).loadGraphic(Paths.image('peashooter/gate'));
			gate.setGraphicSize(Std.int(gate.width * 1.2));
			gate.updateHitbox();
			gate.antialiasing = true;
			gate.scrollFactor.set(0.98, 0.98);
			gate.x += 25;
			gate.active = false;
			add(gate);

			var stageFront:FlxSprite = new FlxSprite(-225, 40).loadGraphic(Paths.image('peashooter/grass'));
			...
			{
				var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('peashooter/redsky'));
				bg.antialiasing = true;
				bg.scrollFactor.set(0.9, 0.9);
				bg.active = true;
				bg.visible = false;
		...
		else if (SONG.song.toLowerCase() == 'bonus-song' || SONG.song.toLowerCase() == 'glitch')
		{
			...
			var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('peashooter/sky_night'));
			bg.antialiasing = true;
			bg.scrollFactor.set(0.9, 0.9);
			bg.active = false;
			add(bg);

			var stageHills:FlxSprite = new FlxSprite(-225, -130).loadGraphic(Paths.image('peashooter/hills_night'));
			stageHills.setGraphicSize(Std.int(stageHills.width * 1.25));
			stageHills.updateHitbox();
			stageHills.antialiasing = true;
			stageHills.scrollFactor.set(1, 1);
			...
			var gate:FlxSprite = new FlxSprite(-200, 47).loadGraphic(Paths.image('peashooter/gate_night'));
			gate.setGraphicSize(Std.int(gate.width * 1.2));
			gate.updateHitbox();
			gate.antialiasing = true;
			gate.scrollFactor.set(0.98, 0.98);
			...
			var stageFront:FlxSprite = new FlxSprite(-220, 40).loadGraphic(Paths.image('peashooter/grass_night'));
			stageFront.setGraphicSize(Std.int(stageFront.width * 1.2));
			stageFront.updateHitbox();
			stageFront.antialiasing = true;
			stageFront.scrollFactor.set(0.9, 0.9);
		...
		else if (SONG.song.toLowerCase() == 'furiosity' || SONG.song.toLowerCase() == 'cheating')
		{
			...
			{
				case 'cheating':
					bg.loadGraphic(Paths.image('peashooter/cheater'));
				default:
					bg.loadGraphic(Paths.image('peashooter/redsky'));
			}
			add(bg);
		    // below code assumes shaders are always enabled which is bad
			var testshader:Shaders.GlitchEffect = new Shaders.GlitchEffect();
		...
		else if (SONG.song.toLowerCase() == 'blocked' || SONG.song.toLowerCase() == 'corn-theft' || SONG.song.toLowerCase() == 'maze' || SONG.song.toLowerCase() == 'splitathon' || SONG.song.toLowerCase() == 'screwed')
		{
			defaultCamZoom = 0.9;
			...
			var skyType:String = curStage == 'bambiFarmNight' ? 'peashooter/sky_night' : 'peashooter/sky';
			if(curStage == 'bambiFarmSunset')
			{
				skyType = 'peashooter/sky_sunset';
			}
			...
			var hills:FlxSprite = new FlxSprite(-250, -200).loadGraphic(Paths.image('snow_pea/orangey hills'));
			hills.antialiasing = true;
			hills.scrollFactor.set(0.7, 0.7);
			hills.active = false;

			var farm:FlxSprite = new FlxSprite(200, 100).loadGraphic(Paths.image('snow_pea/funfarmhouse'));
			farm.antialiasing = true;
			farm.scrollFactor.set(0.9, 0.9);
			farm.active = false;
			
			var foreground:FlxSprite = new FlxSprite(-400, 550).loadGraphic(Paths.image('snow_pea/grass lands'));
			foreground.antialiasing = true;
			foreground.scrollFactor.set(1, 1);
			foreground.active = false;
			
			var cornSet:FlxSprite = new FlxSprite(-350, 325).loadGraphic(Paths.image('snow_pea/Cornys'));
			cornSet.antialiasing = true;
			cornSet.scrollFactor.set(1, 1);
			cornSet.active = false;

			var cornSet3:FlxSprite = new FlxSprite(320, 325).loadGraphic(Paths.image('snow_pea/Cornys'));
			cornSet3.antialiasing = true;
			cornSet3.scrollFactor.set(1, 1);
			cornSet3.active = false;
			
			var cornSet2:FlxSprite = new FlxSprite(1050, 325).loadGraphic(Paths.image('snow_pea/Cornys'));
			cornSet2.antialiasing = true;
			cornSet2.scrollFactor.set(1, 1);
			cornSet2.active = false;
			
			var fence:FlxSprite = new FlxSprite(-350, 250).loadGraphic(Paths.image('snow_pea/crazy fences'));
			fence.antialiasing = true;
			fence.scrollFactor.set(0.98, 0.98);
			fence.active = false;

			var sign:FlxSprite = new FlxSprite(0, 500).loadGraphic(Paths.image('snow_pea/sign'));
		...
		{
			default:
				dadmirror = new Character(350, 400, "peashooter-angey");
		...
		switch (SONG.player2)
		{
			...
			case 'peashooter-annoyed':
			    {
				    dad.y += 250;
				    dad.x += 70;
				}
		    case 'peashooter-angey':
			    {
				    dad.y += 275;
				    dad.x += 70;
			...
			case 'peashooter-splitathon':
				{
					dad.x += 100;
					dad.y += 280;
				}
			case 'snowpea-splitathon':
				{
					dad.x += 95;
					dad.y += 275;
				}
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'house':
				credits = 'Original Song made by MoldyGH';
			case 'insanity':
				credits = 'Original Song made by MoldyGH';
			case 'furiosity':
				credits = 'Original Song made by MoldyGH';
		...
		if (curSong.toLowerCase() == 'splitathon')
		{
			...
			preload('splitathon/SnowPea_WaitWhatNow');
			preload('splitathon/SnowPea_ChillingWithTheCorn');
		}
		if (curSong.toLowerCase() == 'insanity')
		{
			preload('peashooter/redsky');
			preload('peashooter/redsky_fix_attempt');
		}
		...
		iconP2 = new HealthIcon(SONG.player2 == "snow-pea" ? "snow-pea" : SONG.player2, false);
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			if (((SONG.player2 == "peashooter-angey" || SONG.player2 == "snowpea-3d") && player == 0) || (((SONG.player1 == "peashooter-angey" || SONG.player1 == "snowpea-3d") || characteroverride == "peashooter-angey" || characteroverride == 'snowpea-3d') && player == 1))
			{
	...
	override public function update(elapsed:Float)
	{
		...
		if (SONG.song.toLowerCase() == "splitathon")
		{
			...
			{
				case 4736:
					splitathonExpression('lookup', 180, 387);
				case 4800:
					FlxG.camera.flash(FlxColor.WHITE, 1);
					splitathonExpression('backup', -160, 330);
					addSplitathonChar("snowpea-splitathon");
					if (SNOWPEACUTSCENEICONHURHURHUR == null)
					{
						SNOWPEACUTSCENEICONHURHURHUR = new HealthIcon("snow-pea", false);
						SNOWPEACUTSCENEICONHURHURHUR.y = healthBar.y - (SNOWPEACUTSCENEICONHURHURHUR.height / 2);
						add(SNOWPEACUTSCENEICONHURHURHUR);
						SNOWPEACUTSCENEICONHURHURHUR.cameras = [camHUD];
						SNOWPEACUTSCENEICONHURHURHUR.x = -100;
						FlxTween.linearMotion(SNOWPEACUTSCENEICONHURHURHUR, -100, SNOWPEACUTSCENEICONHURHURHUR.y, iconP2.x, SNOWPEACUTSCENEICONHURHURHUR.y, 0.3);
						new FlxTimer().start(0.3, FlingCharacterIconToOblivionAndBeyond);
					}
				case 5824:
					FlxG.camera.flash(FlxColor.WHITE, 1);
					splitathonExpression('snowpea-what', -180, 330);
					addSplitathonChar("peashooter-splitathon");
					iconP2.animation.play("peashooter", true);
				case 6080:
					FlxG.camera.flash(FlxColor.WHITE, 1);
					splitathonExpression('end', -180, 330);
					addSplitathonChar("snowpea-splitathon");
					iconP2.animation.play("snow-pea", true);
				case 8384:
					FlxG.camera.flash(FlxColor.WHITE, 1);
					splitathonExpression('snowpea-corn', -180, 330);
					addSplitathonChar("peashooter-splitathon");
					iconP2.animation.play("peashooter", true);
			}
		}
		if (SONG.song.toLowerCase() == 'insanity')
		{
			switch (curStep)
			{
				...
				case 1176:
					...
					dad.visible = false;
					dadmirror.visible = true;
					curbg.loadGraphic(Paths.image('peashooter/redsky_fix_attempt'));
					curbg.visible = true;
				case 1180:
					dad.visible = true;
					dadmirror.visible = false;

					dad.frames = Paths.getSparrowAtlas('peashooter/HolyFubeepWhatJustHappened');
		...
		if (generatedMusic)
			{
				...
				{	
					...
					if (!daNote.mustPress && daNote.wasGoodHit)
					{
						...
						if(dad.curCharacter == 'snow-pea' || dad.curCharacter == 'snowpea-splitathon' ||  dad.curCharacter == 'snowpea-3d' ||  dad.curCharacter == 'snowpea-angey')
					    {
						   FlxG.camera.shake(0.0050, 0.1);
						   camHUD.shake(0.0040, 0.1);
					    }
		...
		if (FlxG.keys.justPressed.TWO)
		{
			...
			SNOWPEACUTSCENEICONHURHURHUR.cameras = [camHUD];
			SNOWPEACUTSCENEICONHURHURHUR.x = -100;
			FlxTween.linearMotion(SNOWPEACUTSCENEICONHURHURHUR, -100, SNOWPEACUTSCENEICONHURHURHUR.y, iconP2.x, SNOWPEACUTSCENEICONHURHURHUR.y, 0.3, true, {ease: FlxEase.expoInOut});
			new FlxTimer().start(0.3, FlingCharacterIconToOblivionAndBeyond);
		}
		#end
		if (updatevels)
		{
			...
			if (SNOWPEACUTSCENEICONHURHURHUR != null)
			{
				SNOWPEACUTSCENEICONHURHURHUR.x += stupidx;
				SNOWPEACUTSCENEICONHURHURHUR.y += stupidy;
			}
		}
	}
	...
	{
		iconP2.animation.play("snow-pea", true);
		SNOWPEACUTSCENEICONHURHURHUR.animation.play(SONG.player2, true, false, 1);
		stupidx = -5;
		stupidy = -5;
		updatevels = true;
	}
		...
		if (camFollow.x != dad.getMidpoint().x + 150 && focusondad || camFollow.y != dad.getMidpoint().y - 100 && focusondad)
		{
			...
			switch (dad.curCharacter)
			{
				...
				case 'senpai-angry':
					camFollow.y = dad.getMidpoint().y - 430;
					camFollow.x = dad.getMidpoint().x - 100;
				case 'peashooter-angey' | 'snowpea-3d':
					camFollow.y = dad.getMidpoint().y;
			}
	...
	function endSong():Void
	{
		...
		if (isStoryMode)
		{
			...
			else
			{
				...
				{
					case 'corn-theft':
						LoadingState.loadAndSwitchState(new VideoState('assets/videos/MazeCutscene.webm', new PlayState()), false);
					default:
		...
		function goodNoteHit(note:Note):Void
			{
				if (!note.wasGoodHit)
				{
					...
					if(boyfriend.curCharacter == 'snow-pea')
			        {
				       FlxG.camera.shake(0.0040, 0.1);
				       camHUD.shake(0.0035, 0.1);
			        }
			        boyfriend.playAnim('sing' + fuckingDumbassBullshitFuckYou, true);
					if (UsingNewCam)
			        {
	...
	public function splitathonExpression(expression:String, x:Float, y:Float):Void
	{
		if (SONG.song.toLowerCase() == 'splitathon')
		{
			...
			switch (expression)
			{
				...
				case 'snowpea-what':
					thing.frames = Paths.getSparrowAtlas('splitathon/SnowPea_WaitWhatNow');
					thing.animation.addByPrefix('uhhhImConfusedWhatsHappening', 'what', 24);
					thing.animation.play('uhhhImConfusedWhatsHappening');
				case 'snowpea-corn':
					thing.frames = Paths.getSparrowAtlas('splitathon/SnowPea_ChillingWithTheCorn');
